...
double DkSkewEstimator::getSkewAngle()
{
    if (!mMatImg.empty()) {
        ...
        cv::Mat edgeMapHor = computeEdgeMap(separabilityHor, mSepThr * max, dir_horizontal);
        ...
        cv::Mat edgeMapVer = computeEdgeMap(separabilityVer, mSepThr * max, dir_vertical);
...
cv::Mat DkSkewEstimator::computeEdgeMap(cv::Mat separability, double thr, int direction)
{
    ...
    if (direction == dir_horizontal) {
        int progressStep = separability.rows - 2 * H2 - 2 * mKMax;
        int lastValue = mProgress->value();

        float *p;
        for (int r = H2 + mKMax; r < separability.rows - H2 - mKMax; r++) {
            mProgress->setValue(lastValue + qRound(5.0 * (r - H2 - mKMax) / (double)progressStep));
            if (mProgress->wasCanceled())
                break;
        ...
        int progressStep = separability.rows - 2 * W2 - 2 * mKMax;
        int lastValue = mProgress->value();

        float *p;
        for (int r = W2; r < separability.rows - W2; r++) {
            mProgress->setValue(lastValue + qRound(5.0 * (r - W2 - mKMax) / (double)progressStep));
            if (mProgress->wasCanceled())
                break;

            p = separability.ptr<float>(r);
            for (int c = H2 + mKMax; c < separability.cols - H2 - mKMax; c++) {
                if (p[c] > thr) {
                    tmpStatus = 1;
                    for (int k = -mKMax; k <= mKMax; k++) {
                        if (k == 0)
...
QVector<QVector3D> DkSkewEstimator::computeWeights(cv::Mat edgeMap, int direction)
{
    ...
    for (size_t i = 0; i < lines.size(); i++) {
        ...
        if (direction == dir_horizontal) {
            ...
            while (qAbs(x1 - x2) > mMinLineProjLength && K < nIter) {
                    ...
                    for (int y1i = 0; y1i < yrPoss1.size(); y1i++) {
                        ...
                        for (int y2i = 0; y2i < yrPoss2.size(); y2i++) {
                            ...
                            for (int xi = x1; xi <= x2; xi++)
                                for (int yi = -mEpsilon; yi <= mEpsilon; yi++) {
