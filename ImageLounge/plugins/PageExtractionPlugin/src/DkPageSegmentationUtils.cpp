...
void PageExtractor::findPage(cv::Mat img, float scale, std::vector<DkPolyRect> &rects)
{
    ...
    cv::Mat lineImg;
    int accMin = (int)(mHoughPeakThresholdRel * std::min(bw.size().width, bw.size().height));
    std::vector<HoughLine> lines = houghTransform(bw, 1, (float)(CV_PI / 180.0), accMin, mMaxLinesHough);
    if (lines.empty()) {
    ...
    for (size_t i = 0; i < lines.size() - 1; i++) {
        for (size_t j = i + 1; j < lines.size(); j++) {
            ...
            if (angleDiff(lines[i].mAngle, lines[j].mAngle) < mAngleTolerance
                && std::abs(lines[i].mAcc - lines[j].mAcc) < mAccumTolerance * (lines[i].mAcc + lines[j].mAcc)) {
    ...
    for (size_t i = 0; i < EPs.size(); i++) {
        for (size_t j = i + 1; j < EPs.size(); j++) {
            ...
            if (abs(angleDiff(EPs[i].mTheta_k, EPs[j].mTheta_k) - (CV_PI * 0.5)) < mOrthoTol) {
    ...
    for (const IntermediatePeak &ip : IPs) {
        ...
        for (int i = 0; i < 2; i++) {
            for (int j = 0; j < 2; j++) {
                ...
                if (pointToLineDistance(ip.mEp1.mSpatialLines[i], p) < mCornerGapTol
                    && pointToLineDistance(ip.mEp2.mSpatialLines[j], p) < mCornerGapTol) {
                    corners.push_back(p);
                }
            }
        }
    ...
    for (const Rectangle &c : candidates) {
        ...
        for (int i = 0; i < 4; i++) {
            if (cv::norm(c.mCorners[i] - c.mCorners[(i + 1) % 4]) < minRelSideLength * smallerSide) {
                largeEnough = false;
            }
        }
        if (!largeEnough) {
    ...
    if (rectangles.size() > mNumFinalRects) {
        rectangles.erase(rectangles.begin() + mNumFinalRects, rectangles.end());
    }
