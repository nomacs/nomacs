...
public:
    ...
    /**
     * @brief set the current theme but do not load or apply it (restart will apply)
     * @param themeName see:getAvailableThemes()
     */
    void setCurrentTheme(const QString &themeName) const;

    /**
     * @brief read external/user stylesheet and preprocess it but do not apply it
     * @param themeName see:getAvailableThemes();
     * @return stylesheet with preprocessor line/block macros removed
     * @note you would only use this to reload color settings without applying the theme
     */
    ...
    /**
     * @param themeName internal theme name
     * @return human-readable theme name
     */
    QString cleanThemeName(const QString &themeName) const;

    /**
     * @return application palette before any of our modifications
     */
    ...
    /**
     * @return name of theme for setWidgetStyle()
     */
...
protected:
    ...
    bool mOurPaletteChange = false;
    QPalette mSystemPalette; // window manager palette, never modified by us
    QPalette mOurPalette; // modified palette
