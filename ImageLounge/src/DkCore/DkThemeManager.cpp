    ...
    : mSystemPalette(qApp->palette())
{
    ...
    // we connect this after setStylePlugin because that can trigger a palette change
...
QString DkThemeManager::loadTheme(const QString &themeName) const
{
    ...
    return loadStylesheet(tfi.absoluteFilePath());
}
...
void DkThemeManager::applyTheme()
{
    ...
    cssString += loadStylesheet(":/nomacs/stylesheet.css");
    cssString += loadTheme(getCurrentThemeName());
    ...
    // and some missing CSS capbilities
    ...
    if (isSystemTheme()) {
        //
        // Change qApp->palette() to recolor all widgets. Attempts to follow the
        // existing palette contrast ratios.
        //
        // Results depend on the style plugin since the usage of palette can differ,
        // styles can also draw native widgets and ignore palette completely
        //
        // WindowsVista: buttons and control colors cannot be changed
        // Mac: AlternateBackground doesn't match palette in dark theme
        //
...
#if QT_VERSION >= QT_VERSION_CHECK(6, 8, 0)
    // sets a dark/light hint to window manager; on Windows/Mac this alters the window title bar
    qInfo() << "[theme] set color scheme hint:" << scheme;
    hints->setColorScheme(scheme);
#endif

    // note: qApp->palette() must remain unchanged so it always refers to the system palette
    mOurPaletteChange = true;
    mOurPalette = palette.output();

    qApp->setPalette(mOurPalette, "QObject");
    qApp->setPalette(mOurPalette, "QMenu");
...
QString DkThemeManager::loadStylesheet(const QString &path) const
{
    QString cssString;
    QFile file(path);

    if (file.open(QFile::ReadOnly)) {
        cssString = preprocess(file.readAll());
...
QString DkThemeManager::replaceMacros(const QString &cssString, const ColorBinding &colors) const
{
    ...
    do {
        ...
        for (const Macro &m : macros) {
            end = cssString.indexOf(m.name, begin);
            if (end >= 0) {
                macro = &m;
                break;
            }
        ...
        outCss += cssString.mid(begin, end - begin); // chunk before macro started

        int argsIn = cssString.indexOf('(', end) + 1;
        int argsOut = cssString.indexOf(')', argsIn);

        QStringList args = cssString.mid(argsIn, argsOut - argsIn).split(',');

        // qDebug() << macro->name << args << macro->eval(args);
        outCss += macro->eval(args);

        begin = argsOut + 1;

    } while (end >= 0);
    outCss += cssString.mid(begin);
    return outCss;
}
