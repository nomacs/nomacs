...
QImage DkBasicLoader::panTilt(QImage img) {

	cv::Mat blurImg = DkImage::qImage2Mat(img);

	// blur plane by plane
	std::vector<cv::Mat> planes;
	cv::split(blurImg, planes);

	//// dist image based on a rect - note hard coded, so use images > 900 x 600
	//cv::Mat distImg(blurImg.size(), CV_8UC1);
	//distImg = 255;
	//cv::Mat roi(distImg, cv::Rect(100, 500, 800, 100));
	//roi = 0;

	// dist image based on 'some' threshold
	cv::Mat distImg;
	cv::threshold(planes.at(0), distImg, 40, 255, THRESH_BINARY);
	cv::distanceTransform(distImg, distImg, CV_DIST_L2, 3);
	cv::normalize(distImg, distImg, 1.0f, 0.0f, NORM_MINMAX);

	DkTimer dt;

	for (size_t idx = 0; idx < planes.size(); idx++)
		planes.at(idx) = blurPanTilt(planes.at(idx), distImg, 140);		// 140 is the maximal blurring kernel size

	qDebug() << "blurring takes: " << QString::fromStdString(dt.getTotal());

	cv::merge(planes, blurImg);

	return DkImage::mat2QImage(blurImg);
}

Mat DkBasicLoader::blurPanTilt(Mat src, Mat depthImg, int maxKernel) {
	...
	for (int rIdx = 0; rIdx < src.rows; rIdx++) {
		...
		for (int cIdx = 0; cIdx < src.cols; cIdx++) {
			...
			int ks = qRound(depthPtr[cIdx]*maxKernel*0.5f);
