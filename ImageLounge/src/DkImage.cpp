...
void DkImageLoader::copyImageToTemp(QFileInfo tmpPath) {
	...
	if (!tmpPath.exists()) {
		emit showInfoSignal(tr("Sorry, %1 existiert nicht...").arg(tmpPath.absoluteFilePath()));
		return;
	}

	QFileInfo destFileInfo(tmpPath.absoluteFilePath(), currentImage->file().fileName());
	
	bool copied = true;
	
	if (!destFileInfo.exists()) {
		QFile cFile(currentImage->file().absoluteFilePath());
