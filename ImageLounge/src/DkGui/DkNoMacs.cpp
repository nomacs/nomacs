    ...
    : DkNoMacs(parent, flags)
{
    ...
    // frameless window simply goes on the same monitor as normal window
    QScreen *screen = qApp->screenAt(geometry().topLeft());
    setGeometry(screen->availableGeometry());

    // TODO: if this is acceptable, remove menu_view_monitors, chooseMonitor(), DkMonitorConfigDialog, etc
#ifdef CHOOSE_MONITOR
#if QT_VERSION < QT_VERSION_CHECK(6, 0, 0)
...
void DkNoMacsFrameless::chooseMonitor(bool force)
{
    ...
    qInfo() << "[chooseMonitor] set geometry:" << geometry() << "windowState:" << windowState();
}
#endif
